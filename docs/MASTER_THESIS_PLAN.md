...
| 2.5 | 损失函数与评价指标 | Define only measures the thesis uses. | What do accuracy and efficiency metrics each mean? | Verified training/evaluation protocol. If the actual loss is absent from all verified method/training sources, Section 2.5.1 may proceed as a theory-only definition with the concrete thesis protocol explicitly deferred; it must not invent or infer a Work1/Work2 loss. Controlled backfill is required once a final verified protocol becomes available. | Metric-definition table if necessary. | PLANNED |
| 2.5.1 | 分割损失函数 | Cover actually used loss functions. | What optimization objective is truly used? | Verified method/training source. If the actual loss is absent from all verified method/training sources, this section may proceed as a theory-only definition with the concrete thesis protocol explicitly deferred; it must not invent or infer a Work1/Work2 loss. Controlled backfill is required once a final verified protocol becomes available. | None required. | PLANNED |
